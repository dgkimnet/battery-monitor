...
CRON_LINE="$INTERVAL PATH=$(quote_cron_value "$CRON_PATH") BATTERY_MONITOR_API_URL=$(quote_cron_value "$BATTERY_MONITOR_API_URL") BATTERY_MONITOR_DEVICE_ID=$(quote_cron_value "$DEVICE_ID") BATTERY_MONITOR_API_TOKEN=$(quote_cron_value "$TOKEN") $(quote_cron_value "$PYTHON_BIN") $(quote_cron_value "$SCRIPT_DIR/battery_collector.py") >> /tmp/battery-monitor.log 2>&1"
